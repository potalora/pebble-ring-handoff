...
`hermes pebble setup` prepares an owner-only receiver runtime, adds plugin settings on unmanaged Hermes installations, checks one ingress recovery, and creates one five-minute watchdog job. It preserves an existing token, database, and job when rerun. A managed Hermes configuration needs its operator to add the settings and rerun setup. The setup command displays the webhook URL and the token file path; it does not display the token.
...
Run `hermes pebble recover` only after the operator requests ingress recovery. The watchdog created by an explicit setup run is standing authorization for its scheduled owned-ingress checks. Recovery can restart the receiver and replace the Quick Tunnel, which changes the webhook URL. Check the returned status once. A busy, refused, or unverified result is not a successful recovery; do not retry automatically. If the URL changes, give the operator the new URL so they can update the Ring app's webhook setting without changing its authorization header or payload mode. Confirm delivery with a harmless physical capture and a response in its original Discord thread.
